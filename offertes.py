    ...
    with tab_crm:
        if offertes.empty:
            ...
            if sub.empty:
                ...
                st.divider()
                st.subheader("💶 Verstuur naar Balans")
                if not balans_koppeling.koppeling_beschikbaar():
                    st.caption(
                        "Niet geconfigureerd: voeg `balans_sheet_id` toe aan de secrets "
                        "(zelfde sheet als de Balans-app) om dit te activeren."
                    )
                else:
                    goedgekeurd = sub[sub["status"] == "Goedgekeurd"]
                    nog_te_versturen = goedgekeurd[
                        ~goedgekeurd["id"].apply(lambda i: balans_koppeling.is_verstuurd(int(i)))
                    ]
                    if goedgekeurd.empty:
                        st.caption("Nog geen goedgekeurde offertes met deze filters.")
                    elif nog_te_versturen.empty:
                        st.caption("Alle goedgekeurde offertes met deze filters staan al in Balans. ✅")
                    else:
                        bc1, bc2 = st.columns([2, 1.3])
                        with bc1:
                            offerte_keuze = st.selectbox(
                                "Offerte", nog_te_versturen["id"].tolist(),
                                format_func=lambda i: (
                                    f"{nog_te_versturen.set_index('id').loc[i, 'nummer']} "
                                    f"({helpers.euro(nog_te_versturen.set_index('id').loc[i, 'totaalprijs'])})"
                                ),
                                key="balans_offerte_keuze",
                            )
                        with bc2:
                            naam_a, naam_b = balans_koppeling.haal_namen()
                            wie = st.selectbox(
                                "Omzet toewijzen aan", ["a", "b", "samen"],
                                format_func=lambda w: {"a": naam_a, "b": naam_b, "samen": "Samen"}[w],
                                key="balans_wie_keuze",
                            )

                        geschatte_materiaalkost = float(
                            nog_te_versturen.set_index("id").loc[offerte_keuze, "materiaalkost"] or 0
                        )
                        bc3, bc4 = st.columns([1, 1])
                        with bc3:
                            materiaalkost_mee = st.checkbox(
                                "Materiaalkost ook meesturen (als schatting)",
                                value=geschatte_materiaalkost > 0,
                                key="balans_materiaal_checkbox",
                            )
                        with bc4:
                            materiaalkost_bedrag = st.number_input(
                                "Bedrag materiaalkost (€) - pas aan indien je de echte prijs al kent",
                                min_value=0.0, step=1.0, format="%.2f",
                                value=geschatte_materiaalkost,
                                disabled=not materiaalkost_mee,
                                key="balans_materiaal_bedrag",
                            )

                        if st.button("Versturen naar Balans", type="primary", key="balans_verstuur_btn"):
                            rij = nog_te_versturen.set_index("id").loc[offerte_keuze].to_dict()
                            rij["id"] = offerte_keuze
                            ids = balans_koppeling.verstuur_offerte_naar_balans(
                                rij, wie,
                                materiaalkost_bedrag=materiaalkost_bedrag if materiaalkost_mee else None,
                            )
                            if ids:
                                st.success(
                                    f"Verstuurd naar Balans: {len(ids)} transactie(s) "
                                    f"(omzet{' + materiaalkost (geschat)' if len(ids) > 1 else ''})."
                                )
                                st.rerun()
                            else:
                                st.warning("Niets om te versturen (bedrag is 0).")
                        st.caption(
                            "De materiaalkost is een schatting uit de offerte. Zodra de echte "
                            "aankoopfactuur binnenkomt, pas je dit bedrag best aan (of verwijder je "
                            "deze post en voeg je de echte factuur toe) in de Transacties-pagina van "
                            "Balans zelf, zodat het saldo klopt."
                        )
